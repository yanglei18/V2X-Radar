...
class VoxelPostprocessor(BasePostprocessor):
    def __init__(self, anchor_params, params, train):
        super(VoxelPostprocessor, self).__init__(anchor_params, params, train)
        # Support multiple anchor sizes: if 'sizes' exists, calculate num from sizes and angles
        if 'sizes' in self.params['anchor_args']:
            sizes = self.params['anchor_args']['sizes']
            r = self.params['anchor_args']['r']
            self.anchor_num = len(sizes) * len(r)
        else:
            # Backward compatibility: use 'num' if 'sizes' not specified
    ...
    def generate_anchor_box(self):
        ...
        # Support multiple anchor sizes
        if 'sizes' in self.params['anchor_args']:
            sizes = self.params['anchor_args']['sizes']  # [[l1,w1,h1], [l2,w2,h2], ...]
        else:
            # Backward compatibility: single size
            sizes = [[self.params['anchor_args']['l'], 
                     self.params['anchor_args']['w'], 
                     self.params['anchor_args']['h']]]
        ...
        # Generate anchors for each size and each angle
        for size_idx, (l, w, h) in enumerate(sizes):
            for angle_idx, angle in enumerate(r):
                cx_tiled = cx[..., np.newaxis]
                cy_tiled = cy[..., np.newaxis]
                cz_tiled = np.ones_like(cx_tiled) * -1.0
                w_tiled = np.ones_like(cx_tiled) * w
                l_tiled = np.ones_like(cx_tiled) * l
                h_tiled = np.ones_like(cx_tiled) * h
                r_tiled = np.ones_like(cx_tiled) * angle
                
                if self.params['order'] == 'hwl':
                    anchor = np.stack([cx_tiled, cy_tiled, cz_tiled, h_tiled, w_tiled, l_tiled, r_tiled], axis=-1)
                elif self.params['order'] == 'lhw':
                    anchor = np.stack([cx_tiled, cy_tiled, cz_tiled, l_tiled, h_tiled, w_tiled, r_tiled], axis=-1)
                else:
                    sys.exit('Unknown bbx order.')
                anchor_list.append(anchor)
        
        # Concatenate all anchors: (H, W, num_sizes*num_angles, 7)
        anchors = np.concatenate(anchor_list, axis=2)
        return anchors
            ...
            if len(boxes3d) != 0:
                ...
                projected_boxes3d = \
                    box_utils.project_box3d(boxes3d_corner,
                                            transformation_matrix.to(boxes3d.device))
