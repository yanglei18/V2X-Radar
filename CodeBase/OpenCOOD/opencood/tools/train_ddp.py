...
def main():
    ...
    for epoch in range(init_epoch, max(epoches, init_epoch)):
        ...
        if epoch % hypes['train_params']['eval_freq'] == 0:
            ...
            if valid_ave_loss < lowest_val_loss:
                ...
                if opt.rank == 0:
                    torch.save(model_without_ddp.state_dict(), os.path.join(saved_path, 'net_epoch%d_%.4f.pth' % (epoch + 1, lowest_val_loss)))

        print("history validation loss: ", valid_loss_list)
        print("lowest_val_epoch: ", lowest_val_epoch)
    ...
    if opt.rank == 0:
        ...
        if run_test:
            ...
            cmd = f"python opencood/tools/inference_modify.py --model_dir {saved_path} --fusion_method {fusion_method}"
