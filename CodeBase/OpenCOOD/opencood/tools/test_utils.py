...
def inference_late_fusion(batch_data, model, dataset):
    # loop through all cavs and get the output of the model
    output_dict = OrderedDict()
    for cav_id, cav_content in batch_data.items():
        output_dict[cav_id] = model(cav_content)

    # post process all cavs' output of the model
...
def inference_no_fusion(batch_data, model, dataset, single_gt=False):
    ...
    # but batch_data havs all cavs, 
    # because we need the gt box inside.
    output_dict_ego = OrderedDict()
    if single_gt: batch_data = {'ego': batch_data['ego']}
    output_dict_ego['ego'] = model(batch_data['ego'])

    # we only need to post process ego's output, 
    # while gt box is from batch_data (if single_gt is True/False)
    pred_box_tensor, pred_score, gt_box_tensor = \
        dataset.post_process_no_fusion(batch_data, output_dict_ego)
    ...
    return return_dict, output_dict_ego

def inference_early_fusion(batch_data, model, dataset):
    # only care about ego's output
    output_dict = OrderedDict()
    cav_content = batch_data['ego']
    output_dict['ego'] = model(cav_content)
    
    # post process ego's output
    pred_box_tensor, pred_score, gt_box_tensor = \
        dataset.post_process(batch_data, output_dict)
    ...
    # if depth items are in output_dict['ego'], add them to return_dict
    if "depth_items" in output_dict['ego']:
        return_dict.update({"depth_items" : output_dict['ego']['depth_items']})
    return return_dict, output_dict

def inference_intermediate_fusion(batch_data, model, dataset):
    # same with early fusion
    return_dict, output_dict = inference_early_fusion(batch_data, model, dataset)
    return return_dict, output_dict
