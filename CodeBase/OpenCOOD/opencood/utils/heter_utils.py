...
def assign_modality_2(root_dir="dataset/OPV2V", output_path="opencood/logs/heter_modality_assign/opv2v_4modality.json"):
    np.random.seed(303)
    splits = ['train', 'validate'] # splits = ['train', 'test', 'validate']
    scenario_cav_modality_dict = OrderedDict()
    ...
    for split in splits:
        ...
        for scenario_folder in scenario_folders:
            ...
            perm = np.random.permutation(2) + 1  # perm = np.random.permutation(4) + 1 
            for j, cav_id in enumerate(cav_list):
                scenario_cav_modality_dict[scenario_name][cav_id] = 'm'+str(perm[j%2]) # 'm'+str(perm[j%4]) # m1 or m2 or m3 or m4
