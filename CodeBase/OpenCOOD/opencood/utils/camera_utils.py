...
def load_camera_data(camera_files, preload=True):
    ...
    for idx, camera_file in enumerate(camera_files):
        ...
        if not os.path.exists(camera_file):
            print(f"Image file {camera_file} does not exist.")
            continue
        ...
        try:
            ...
            if preload:
                ...
                if camera_data is None:
                    print(f"Image {camera_file} is None before copy().") 
                    camera_data = camera_data.copy()
                    print(f"Copied image {camera_file} successfully.")  
            camera_data_list.append(camera_data)
        except (IOError, OSError) as e:
            print(f"Error loading image {camera_file} at index {idx}: {e}")
